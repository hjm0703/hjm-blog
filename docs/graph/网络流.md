...
> [!question] 问题引入
> 
> 对于每一个点赋予一个点权，并且设置一些依赖项，问选择边权最大值。

这里我们对于所有依赖项再网络流中连接，然后对于点权为正的连接其和源点，负数连接其和汇点，边权取反。

那么此时按照最小割模型，就可以完全套用（但是最后答案需要用所有正数边权减去答案，因为求解的是最少去除多少个正边权和最小选择几个负边权的点）。
## 最小费用最大流
...
即我们赋予二分图边一个贡献权值，然后要求权值总和最大。

如果所有权值为正，可以发现一定同时是二分图最大匹配。因此直接连接二分图，然后跑最大费用。

但是如果有边权为负的，就需要设置一些反悔措施，即我们连接带边权的边的同时连接不带边权的边。表示可以为二分图匹配进行贡献但是不对权值贡献。
