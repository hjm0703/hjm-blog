...
一般简写为 `e-Dcc`。其定义为这里连通分量中不包含割边。其实质是划分点。

这里实现逻辑实际上和上面的相同，如果对于一个点 $dfn_x = low_x$ ，那么相当于这个点带领的子树一定需要被重新划分。因此依然使用一个桶维护
...
void tarjan(int x, int edge) {
	...
	for(int i=head[x]; i; i=nxt[i]) {
		...
		if(!dfn[y]) {
			...
			if(low[y] >= dfn[x]) {
				child++;
				if(x != root || child > 1) flag[x] = 1; // 此时 x 为割点
			}
		} else low[x] = min(low[x], dfn[y]);
	}
}
...
我们发现，对于随机一条路径所经过的 "点双" ，一定都是并集中的点。并且这是充要的，证明如下图：
...
这里我们可以建立园方树之后先处理两边到仙人掌中间的最短路。然后在交会的那个环上分类讨论两种路线。实现起来相当麻烦。

%%待未来完成%%

## 有向图连通性

其实这里主要的用处是在竞赛图和 2-SAT。主要还是 2-SAT 。

如果对于图中一个子图他们之中任意两个点都能够互相到达，那么他们就是一个 **强连通分量** 。

实现的原理和上面依然相同，只不过定义从无向边变成了有向边。具体实现：
...
void Tarjan(int x) {
	...
	for(int i=head[x]; i; i=nxt[i]) {
		...
		if(!dfn[y]) {
			Tarjan(y);
			low[x] = min(low[x], low[y]);
		} else if(ins[y]) { // 最特殊的地方，因此有向图的横向边可以存在
			low[x] = min(low[x], dfn[y]);
		}
	}
	if(dfn[x] == low[x]) {
